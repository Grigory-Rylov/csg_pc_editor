...
class ControllerHolderBuilder(
    ...
    fun create(): ModelHolder {
        val model = createControllerModel()
        return ModelHolder(
            model, VertexHolder.createVertexHolder(
    ...
    private fun createControllerModel(): Abstract3dModel {
        ...
        val height = 10.0
        val verticalWall = Cube(33.0, 1.5, 10.0)
            .subtractModel(placeSwitcher(switcher.createSwitcherHole())
                .addModel(usbHole())
        ).move(6.5, 17.5, 2.5)
        return model.addModel(controllerPlace.place(verticalWall))
            .addModel(controllerPlace.place(createHolder()))
    }
    ...
    private fun createHolder(): Abstract3dModel {
        ...
        val body = Cube(controller.width + 3.0, controller.depth + 1, height)
            .moveX(-0.5)
            .subtractModel(
