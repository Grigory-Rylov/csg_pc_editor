...
public class DebugNakedEdgeCmd implements DebugCmd {
    ...
    private DebugNakedEdgeCmd(Polygon polygon, V3d nakedEdgeA, V3d nakedEdgeB, Facet facet,
    ...
    public static DebugNakedEdgeCmd withAllFacets(Polygon polygon, V3d nakedEdgeA, V3d nakedEdgeB, Facet facet,
                                                  List<Facet> allFacets) {
        return new DebugNakedEdgeCmd(polygon, nakedEdgeA, nakedEdgeB, facet, allFacets, new ArrayList<>());
    }

    public static DebugNakedEdgeCmd withAllNearbyFacets(Polygon polygon, V3d nakedEdgeA, V3d nakedEdgeB, Facet facet,
                                                        List<PolygonValidator.PolygonEdge> nearby) {
        return new DebugNakedEdgeCmd(polygon, nakedEdgeA, nakedEdgeB, facet, new ArrayList<>(), nearby);
    }
    ...
    public void render(DebugVisualizer debugVisualizer) {
        ...
        for (Map.Entry<Polygon, List<Facet>> p : nearbyMap.entrySet()) {
            ...
            for (Facet facet : p.getValue()) {
                debugVisualizer.drawDebugFacet(facet,
        ...
        for (Facet f : allFacets) {
            debugVisualizer.drawDebugFacet(f, DbgConfig.LINE_THICKNESS,
                DbgConfig.POINT_THICKNESS, Color.lightGray, Color.WHITE);
        }

        if (polygon != null) {
            debugVisualizer.drawDebugPolygon(polygon, DbgConfig.LINE_THICKNESS,
        ...
        debugVisualizer.drawDebugLine(nakedEdgeA, nakedEdgeB,
