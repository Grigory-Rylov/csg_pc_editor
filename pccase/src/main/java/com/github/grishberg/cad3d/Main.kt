...
class Main(title: String?) : JFrame(title), GLEventListener {
    ...
    private var allModels = mutableMapOf<String, ModelData>()
    private var visibleModels = mutableMapOf<String, ModelData>()

    init {
        title ?: "PC Case Viewer (OpenGL)"

        val models = buildPcCaseModels()
        allModels = models.mapValues { (_, csg) -> csgToModelData(csg) }.toMutableMap()
        visibleModels = allModels.toMutableMap()
        ...
        val checkboxes = createCheckboxes()
        contentPane.add(checkboxes, BorderLayout.NORTH)
    ...
    private fun createCheckboxes(): JPanel {
        ...
        val checkboxes = mutableMapOf<String, JCheckBox>()
        val componentLabels = listOf(
        ...
        for ((key, label) in componentLabels) {
            ...
            cb.addActionListener {
                visibleModels = allModels.filterKeys { k ->
                    checkboxes[k]?.isSelected ?: true
                }.toMutableMap()
                glCanvas?.display()
            }
            checkboxes[key] = cb
